...
filename="carica0312n2_50deg.csv";
Duration = 1.5;
start_timestamp=datetime("now");

S1=serial('COM2'); % assegnazione
...
T2ch.TerminalConfig = 'SingleEnded';
...
function saveData(src, event, fid, c, LEM)
         fprintf(fid,'%d; %d; %02.3f;%6.3f;%6.3f;%6.3f;%6.3f;\n', c(4), c(5), c(6)+event.TimeStamps(13), LEM, mean(event.Data(1:25,1)), mean(event.Data(1:25,2)), mean(event.Data(1:25,3)));
         fprintf(fid,'%d; %d; %02.3f;%6.3f;%6.3f;%6.3f;%6.3f;\n', c(4), c(5), c(6)+event.TimeStamps(38), LEM, mean(event.Data(26:50,1)), mean(event.Data(26:50,2)), mean(event.Data(26:50,3)));
         fprintf(fid,'%d; %d; %02.3f;%6.3f;%6.3f;%6.3f;%6.3f;\n', c(4), c(5), c(6)+event.TimeStamps(63), LEM, mean(event.Data(51:75,1)), mean(event.Data(51:75,2)), mean(event.Data(51:75,3)));
         fprintf(fid,'%d; %d; %02.3f;%6.3f;%6.3f;%6.3f;%6.3f;\n', c(4), c(5), c(6)+event.TimeStamps(88), LEM, mean(event.Data(76:100,1)), mean(event.Data(76:100,2)), mean(event.Data(76:100,3)));
end